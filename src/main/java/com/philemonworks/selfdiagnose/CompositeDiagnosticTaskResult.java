...
public class CompositeDiagnosticTaskResult extends DiagnosticTaskResult {
    ...
    private List<DiagnosticTaskResult> results = new ArrayList<DiagnosticTaskResult>();
    ...
    public List<DiagnosticTaskResult> getResults() {
        return results;
    }

    public int howManyErrors() {
        int sum = 0;
        for (DiagnosticTaskResult each : results) {
            if (!each.isPassed())
                sum++;
        }
        return sum;
     ...
     */
    public void addToResults(List<DiagnosticTaskResult> newResults) {
        if (includeCompositeResult)
            newResults.add(this);
        // add all or mine
